import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";
import { formatDate, formatMoney } from "@/lib/utils";
import { drawLetterhead, drawDocumentFooter } from "./letterhead";
import type { InvoiceItem, PartyType } from "@/lib/types/database";

export type InvoicePdfData = {
  invoice_no: string;
  party_type: PartyType;
  party_name: string;
  party_detail: string | null;
  party_phone: string | null;
  party_address: string | null;
  parent_name: string | null;
  parent_phone: string | null;
  items: InvoiceItem[];
  total: number;
  paid: number;
  balance: number;
  status: string;
  issued_date: string;
  due_date: string | null;
  note: string | null;
};

export function invoicePdfFilename(inv: InvoicePdfData) {
  return `${inv.invoice_no.toLowerCase()}-${slug(inv.party_name)}.pdf`;
}

export function downloadInvoicePdf(inv: InvoicePdfData, schoolName: string) {
  buildInvoicePdf(inv, schoolName).save(invoicePdfFilename(inv));
}

export function buildInvoicePdf(inv: InvoicePdfData, schoolName: string) {
  const doc = new jsPDF();
  drawLetterhead(doc, "Invoice", schoolName);

  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  // A student invoice bills the student; a teacher/staff invoice is a
  // payment voucher in the person's favour.
  doc.text(inv.party_type === "student" ? "Billed to" : "Payable to", 14, 40);
  doc.setFont("helvetica", "normal");
  doc.text(inv.party_name, 14, 46);
  doc.setTextColor(100);
  const partyLines = contactLines(inv);
  let py = 52;
  doc.text(inv.party_detail ?? partyTypeLabel(inv.party_type), 14, py);
  partyLines.forEach((line) => {
    py += 5;
    doc.text(line, 14, py);
  });
  doc.setTextColor(0);

  doc.setFont("helvetica", "bold");
  doc.text(inv.invoice_no, 196, 40, { align: "right" });
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100);
  doc.text(`Issued: ${formatDate(inv.issued_date)}`, 196, 46, { align: "right" });
  if (inv.due_date) doc.text(`Due: ${formatDate(inv.due_date)}`, 196, 52, { align: "right" });
  doc.setTextColor(0);

  // Line-item table starts below whichever column (party block vs. the
  // invoice meta on the right) runs longer.
  const startY = Math.max(66, py + 8);

  autoTable(doc, {
    startY,
    head: [["#", "Description", "Qty", "Unit price", "Amount"]],
    body: inv.items.map((it, i) => [
      String(i + 1),
      it.description,
      String(it.qty),
      formatMoney(it.unit_price),
      formatMoney(it.qty * it.unit_price),
    ]),
    theme: "grid",
    headStyles: { fillColor: [0, 122, 255] },
    columnStyles: {
      0: { cellWidth: 10 },
      2: { cellWidth: 16, halign: "right" },
      3: { cellWidth: 28, halign: "right" },
      4: { cellWidth: 28, halign: "right" },
    },
  });

  let y = (doc as unknown as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 8;

  const totals: Array<[string, string, boolean]> = [
    ["Total", formatMoney(inv.total), false],
    ["Paid to date", formatMoney(inv.paid), false],
    ["Balance due", formatMoney(inv.balance), true],
  ];
  totals.forEach(([label, value, strong]) => {
    doc.setFont("helvetica", strong ? "bold" : "normal");
    doc.setFontSize(strong ? 11 : 10);
    doc.text(label, 140, y);
    doc.text(value, 196, y, { align: "right" });
    y += 6.5;
  });

  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text(`Status: ${inv.status.toUpperCase()}`, 14, y - 6.5);
  doc.setFont("helvetica", "normal");

  if (inv.note) {
    y += 4;
    doc.setFontSize(9.5);
    doc.setTextColor(100);
    doc.text(`Note: ${inv.note}`, 14, y, { maxWidth: 182 });
    doc.setTextColor(0);
  }

  doc.setDrawColor(180);
  doc.line(14, 268, 74, 268);
  doc.line(136, 268, 196, 268);
  doc.setFontSize(9);
  doc.setTextColor(140);
  doc.text("Prepared by", 14, 273);
  doc.text("Authorized signature", 136, 273);
  doc.setTextColor(0);
  drawDocumentFooter(doc);

  return doc;
}

function partyTypeLabel(type: PartyType) {
  return type === "student" ? "Student" : type === "teacher" ? "Teacher" : "Staff";
}

// Contact + parent lines rendered under the party name. Kept short so the
// block stays inside the header area; parent details are student-only.
function contactLines(inv: {
  party_type: PartyType;
  party_phone: string | null;
  party_address: string | null;
  parent_name: string | null;
  parent_phone: string | null;
}) {
  const lines: string[] = [];
  if (inv.party_phone) lines.push(`Phone: ${inv.party_phone}`);
  if (inv.party_address) lines.push(`Address: ${inv.party_address}`);
  if (inv.party_type === "student") {
    const parent = [inv.parent_name, inv.parent_phone].filter(Boolean).join(" · ");
    if (parent) lines.push(`Parent/guardian: ${parent}`);
  }
  return lines;
}

function slug(name: string) {
  return name.trim().toLowerCase().replace(/\s+/g, "-");
}
