...
export function InstallmentsModal({
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      show(t("fee.scheduleSaved"));
      onClose();
    });
  }

  return (
    <Modal open={open} onClose={onClose} title={t("fee.scheduleTitle", { name: year.name })} widthClass="max-w-xl">
      <div className="space-y-4">
        <p className="text-[12.5px] text-text-2">{t("fee.scheduleIntro")}</p>
          ...
          {rows.map((row, i) => (
            <div key={i} className="grid grid-cols-[1fr_150px_90px_32px] gap-2 items-center">
              <Input
                ...
                aria-label={t("fee.installmentName")}
              />
              <Input
                type="date"
                value={row.due_date}
              ...
              <Input
                ...
                placeholder={t("fee.percentPlaceholder")}
                aria-label={t("fee.percentHelp")}
              />
              <button
                type="button"
                ...
                aria-label={t("fee.removeInstallment")}
              >
                <Trash2 size={14} />
              </button>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <Plus size={14} /> {t("fee.addInstallment")}
          </Button>
        ...
        <div>
          <Label className="sr-only">{t("fee.errors")}</Label>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button onClick={save} disabled={pending || total > 100}>
            {pending ? t("common.saving") : t("fee.saveSchedule")}
          </Button>
        </div>
      </div>
    </Modal>
