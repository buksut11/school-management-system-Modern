  ...
  return (
    <div className="r-table subj-table rounded-2xl bg-card backdrop-blur-2xl backdrop-saturate-150 border border-line shadow-card overflow-hidden">
      <div className="r-head flex items-center gap-3 px-5 py-3 border-b border-line text-[11px] font-semibold text-text-2 uppercase tracking-wide">
        ...
        <div className="flex-1 min-w-[150px]">Subject</div>
        <div className="w-32 flex-none scol-dept">Department</div>
        <div className="w-16 flex-none scol-type">Type</div>
        <div className="w-24 flex-none scol-periods">Periods/wk</div>
      ...
      <div className="divide-y divide-line/60">
        {subjects.map((s) => (
          <div key={s.id} className="r-card flex items-center gap-3 px-5 py-3">
            ...
            <div className="r-cell scol-type w-16 flex-none" data-label="Type">
              <Badge tone={s.type === "core" ? "blue" : "gray"}>{s.type === "core" ? "Core" : "Elective"}</Badge>
            </div>
            <div className="r-cell scol-periods w-24 flex-none text-[13px] text-text-2" data-label="Periods/wk">
              {s.periods_per_week}
            </div>
        ...
        {subjects.length === 0 && (
          <div className="py-12 text-center text-[13px] text-text-2">No subjects found.</div>
        )}
      </div>
    </div>
  );
