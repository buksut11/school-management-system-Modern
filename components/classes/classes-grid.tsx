...
export function ClassesGrid({
  ...
  if (classes.length === 0) {
    return <Card className="py-12 text-center text-[13px] text-text-2">No classes found.</Card>;
  }
      ...
      {classes.map((c) => (
        <Card key={c.id} className="p-4">
          <div className="mb-3">
            ...
            <div className="text-[11.5px] text-text-2">{c.room ?? "No room set"}</div>
          </div>
          <div className="space-y-1.5 text-[12.5px] text-text-2 mb-3">
            <div className="flex justify-between">
              <span>Class teacher</span>
              <span className="text-text font-medium">{c.teacher_name ?? "Unassigned"}</span>
            </div>
            <div className="flex justify-between">
              <span>Enrolled</span>
