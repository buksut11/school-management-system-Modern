  ...
  return (
    <div className="r-table dept-table rounded-2xl bg-card backdrop-blur-2xl backdrop-saturate-150 border border-line shadow-card overflow-hidden">
      <div className="r-head flex items-center gap-3 px-5 py-3 border-b border-line text-[11px] font-semibold text-text-2 uppercase tracking-wide">
        ...
        <div className="flex-1 min-w-[160px]">{t("col.department")}</div>
        <div className="w-36 flex-none dcol-head">{t("col.head")}</div>
        <div className="w-24 flex-none dcol-subj">{t("col.subjects")}</div>
        <div className="w-24 flex-none dcol-teachers">{t("col.teachers")}</div>
        <div className="w-24 flex-none dcol-periods">{t("subject.statPeriods")}</div>
      ...
      <div className="divide-y divide-line/60">
        {departments.map((d) => (
          <div key={d.id} className="r-card flex items-center gap-3 px-5 py-3">
            ...
            <div className="r-cell dcol-head w-36 flex-none text-[13px] text-text-2 truncate" data-label={t("col.head")}>
              {d.head_teacher_name ?? t("common.unassigned")}
            </div>
            <div className="r-cell dcol-subj w-24 flex-none text-[13px] text-text-2" data-label={t("col.subjects")}>
              {d.subject_count}
            </div>
            <div className="r-cell dcol-teachers w-24 flex-none text-[13px] text-text-2" data-label={t("col.teachers")}>
              {d.teacher_count}
            </div>
            <div className="r-cell dcol-periods w-24 flex-none text-[13px] text-text-2" data-label={t("subject.statPeriods")}>
              {d.periods_per_week}
            </div>
        ...
        {departments.length === 0 && (
          <div className="py-12 text-center text-[13px] text-text-2">{t("dept.notFound")}</div>
        )}
      </div>
    </div>
  );
